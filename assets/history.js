...
function historyCard(row) {
  ...
  const progress = rawDuration > 0
    ? Math.min(100, Math.round((rawProgress / duration) * 100))
    : 0;
  const statusLabel = rawDuration > 0
    ? (progress >= 95 ? "COMPLETED" : `${progress}% WATCHED`)
    : "RECENTLY VIEWED";
...
if (user) {
  const { data, error } = await supabase.from("watch_history")
    .select("progress_seconds,duration_seconds,updated_at,video_posts(*)")
    .eq("user_id", user.id).order("updated_at", { ascending:false }).limit(40);
  if (error) message.textContent = error.message;
  else {
    const rows = (data || []).filter(row => row.video_posts);
